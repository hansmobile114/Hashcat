...
-(BOOL)textField:(AKTextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string
{
    ...
    if (!isBackSpace){
        ...
        if (string.length == 0){
            textField.backgroundColor = MAIN_COLOR;
...
{
    return [_textFieldCell.textField resignFirstResponder];
}
...
-(void)addNewItemWithString:(NSString *)string completion:(void (^)(void))completion
{
    ...
    {
        if ([self.delegate respondsToSelector:@selector(alertExceedLimitedTags:)]){
            [self.delegate alertExceedLimitedTags:self];
        }
