...
+ (void)show
{
	[[self sharedInstance] loadingViewSetupWithText:nil andShimmering:NO andBlur:YES];
}

+ (void)showWithText:(NSString *)text
{
    [[self sharedInstance] loadingViewSetupWithText:text andShimmering:NO andBlur:YES];

}

+ (void)showWithText:(NSString *)text andShimmering:(BOOL)shimmering andBlurEffect:(BOOL)blur
{
    [[self sharedInstance] loadingViewSetupWithText:text andShimmering:NO andBlur:blur];
}
...
- (void)loadingViewSetupWithText:(NSString *)text andShimmering:(BOOL)shimmering andBlur:(BOOL)blur
{
    if (!loadingImageView)
    {
        ...
        if (text && ![text isEqualToString:@""]) {
            ...
            loaderView.frame = CGRectMake(0, 0, self.frame.size.width, self.frame.size.height);
            CGRect rectLoadView = CGRectMake((self.frame.size.width - loaderBackgroundWidth) / 2,
            ...
            if (shimmering) {
                ...
                shimmeringView.frame = CGRectMake(loadingTextLabelSideMargin,
                                                  loaderView.frame.size.height / 2 + height + 2 * loadingTextLabelBottomMargin + 30,
            ...
            else
            {
                ...
                loadingLabel.frame = CGRectMake(loadingTextLabelSideMargin,
                                                  loaderView.frame.size.height / 2 + height + 2 * loadingTextLabelBottomMargin,
                                                  loaderView.frame.size.width - (loadingTextLabelSideMargin * 2),
                                                  height);
            ...
            loadingLabel.textColor = [UIColor whiteColor];//loadingTextColor;
